...
from crud import (add_product, get_all_products, delete_product,
                  add_product_income, add_product_expense, get_product_inventory)
...
with tab_inc:
    ...
    prods = to_dict_list(get_all_products(kg_id))
    if prods:
...
with tab_dict:
    ...
    with st.form("add_prod_form"):
        ...
        sel_unit = st.selectbox(t("unit_label"), options=units_display)
        unit_value = PRODUCT_UNITS[units_display.index(sel_unit)]
        if st.form_submit_button(t("add_to_dict_btn")):
            add_product(kg_id, n, unit_value, 1.0)
            st.rerun()

    all_p = to_dict_list(get_all_products(kg_id))
    if all_p:
